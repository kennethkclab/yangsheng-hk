...
const tones: Record<string, string> = {
  ...
  youtube: "bg-[#FF0000]",
  douyin: "bg-black",
  xiaohongshu: "overflow-hidden bg-[#FF2442]",
};

const stackIds = ["facebook", "youtube", "douyin", "xiaohongshu"] as const;

export default function FloatingSocial() {
  const stack = stackIds
    .map((id) => siteData.socials.find((item) => item.id === id))
    .filter((item): item is (typeof siteData.socials)[number] => Boolean(item));
  const whatsapp = siteData.socials.find((item) => item.id === "whatsapp") ?? {
    id: "whatsapp",
    label: "WhatsApp",
  ...
  return (
    <>
      <div className="fixed right-0 top-1/2 z-[80] flex -translate-y-1/2 flex-col">
        {stack.map((item) => (
          <a
            key={item.id}
            href={item.href}
            ...
            className={`flex h-11 w-11 items-center justify-center text-white shadow-md ${tones[item.id] ?? "bg-brand-700"}`}
          >
            {icons[item.id]}
          </a>
        ))}
      ...
      <a
        ...
        rel="noopener noreferrer"
        aria-label="WhatsApp"
        className="fixed bottom-5 right-4 z-[80] flex h-14 w-14 items-center justify-center rounded-full bg-[#25D366] text-white shadow-lg md:bottom-8 md:right-8"
      >
