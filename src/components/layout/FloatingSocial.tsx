...
const stack = [
  ...
  },
  {
    ...
    src: `${R2}/images/icons/xiaohongshu.jpg`,
  },
];
...
export default function FloatingSocial() {
  return (
    <>
      <div className="fixed right-2 top-1/2 z-[80] flex -translate-y-1/2 flex-col gap-2.5 md:right-3">
        {stack.map((item) => (
          <a
            key={item.id}
            href={item.href}
            ...
            className="flex h-10 w-10 items-center justify-center overflow-hidden rounded-full shadow-md"
          >
            {item.id === "youtube" ? (
      ...
      <a
        ...
        rel="noopener noreferrer"
        aria-label="WhatsApp"
        className="fixed bottom-5 right-3 z-[80] h-14 w-14 overflow-hidden rounded-full shadow-lg md:bottom-8 md:right-5"
      >
        {/* eslint-disable-next-line @next/next/no-img-element */}
